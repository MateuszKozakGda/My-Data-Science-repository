...
url = "https://www.biznesradar.pl/notowania-historyczne/PKN-ORLEN"
...
if len(argv)>1 and argv[1] == "setup":
    cursor.execute("""CREATE TABLE pkn_stock
                   (
    ...
    try:
        while i<get_last_page_number(soup):
            if i==1:
                ...
                page = requests.get(f"https://www.biznesradar.pl/notowania-historyczne/PKN-ORLEN,{i}")
                soup = BeautifulSoup(page.content, "html.parser")
                table =  soup.find("table", class_="qTableFull")
                rows = table.find_all("tr")
                for row in rows:

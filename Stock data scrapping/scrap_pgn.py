...
url = "https://www.biznesradar.pl/notowania-historyczne/PGNIG"
...
if len(argv)>1 and argv[1] == "setup":
    cursor.execute("""CREATE TABLE pgnig_stock
                   (
            ...
            if i==1:
                ...
                for row in rows:
                    ...
                    if bool(cols):
                        ...
                        print(f"DATE: {date}")
                        cursor.execute(
                            "INSERT INTO pgnig_stock VALUES (?,?,?,?,?,?,?)",
                ...
                page = requests.get(f"https://www.biznesradar.pl/notowania-historyczne/PGNIG,{i}")
                soup = BeautifulSoup(page.content, "html.parser")
                table =  soup.find("table", class_="qTableFull")
                rows = table.find_all("tr")
                for row in rows:

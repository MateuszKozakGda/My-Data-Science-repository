...
#out = tf.compat.v1.get_default_graph().get_tensor_by_name("model/layer10/batch_norm10/beta/Momentum/read:0")
#logits = tf.layers.dense(out, n_outputs,
                          #kernel_initializer=tf.contrib.layers.variance_scaling_initializer(),
                          #name="logits")
#y_proba = tf.nn.softmax(logits, name="y_proba")

xentropy = tf.compat.v1.get_default_graph().get_tensor_by_name("loss/cross_entropy:0")
loss = tf.compat.v1.get_default_graph().get_tensor_by_name("loss/loss:0")

#define trainable variables

output_layer_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="net_output")
...
saving_path = "./minit_checkpoint/minist_model_5_9"
...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
      ...
      for batch in bar:
        X_batch, y_batch = get_random_batch(X_train2, y_train2, batch_size=batch_size)
      ...
      if X_valid2 is not None and y_valid2 is not None:
        feed_dict2 = {X: X_valid2, y: y_valid2}
        ...
        if checks_without_progress > early_stopping_rounds:
          ...
          five_frozen_saver.save(sess, "./minit_checkpoint/minist_model_5_9")
          break
        
        
        
    print("CONFUSION MATRIX")
    feed_dict3 = {X: X_valid2, y: y_valid2}
